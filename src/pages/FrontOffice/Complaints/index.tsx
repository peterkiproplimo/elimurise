...
function Main() {
  ...
  const handleReplySubmit = async (id:any) => {
    ...
    learner.replies = [...(learner.replies || []), reply];

    try{
        
        await ApiService.createReply(id, reply);
  ...
  const handleStatusChange = async (learnerId, newStatus: string) => {
      ...
      try {
          ...
          await ApiService.updateStatus(learnerId, reply);
...
//     setLoading(true);
//     const res = await  ApiService.getReplies(learner._id);
...
// };
  ...
  //     try {
  //       setLoading(true);
  //       const res = await ApiService.getReplies(complaintId);
  //       setReplies(res.replies || []);
  //     } catch (error) {
  //       console.error("Error loading replies:", error);
  //     } finally {
  ...
  const handleButtonClick = () => {
    const allData = getValues(); // get all fields
    console.log(allData);

    const onlyName = getValues("reporter_name"); // get specific field
    console.log("Reporter name:", onlyName);
  };
  ...
  return (
    <>
      ...
      {dialog && !profile ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div>
                    ...
                    <FormInput
                      ...
                      type="text"
                      name="subject"
                      className={`mt-1 w-full rounded-lg border ${errors.subject ? "border-red-500" : "border-gray-300"
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      name="description"
                      rows={4}
                      className={`mt-1 w-full rounded-lg border ${errors.description ? "border-red-500" : "border-gray-300"
                        } focus:ring-2 focus:ring-indigo-500 transition-all p-2`}
                  ...
                  <div>
                    ...
                    {errors.priority && (
                      <div className="mt-2 text-red-500 text-sm">
                        {typeof errors.priority.message ===
                  ...
                  <div>
                    ...
                    <FormInput
                      ...
                      type="text"
                      name="reporterName"
                      className={`mt-1 w-full rounded-lg border ${errors.reporterName ? "border-red-500" : "border-gray-300"
                        } focus:ring-2 focus:ring-indigo-500 transition-all`}
                      placeholder="Reporter Name"
                    />
                    {errors.reporterName && (
                      <div className="mt-2 text-red-500 text-sm">
                        {typeof errors.reporterName.message === "string" &&
                  ...
                  <div>
                    ...
                    {errors.reporterContact && (
                      <div className="mt-2 text-red-500 text-sm">
                        {typeof errors.reporterContact.message === "string" &&
        ...
        <>
          <h2 className="mt-1 text-lg font-medium ">Complaints</h2>
                  ...
                  <Table hover striped className="mt-6">
                    ...
                    <Table.Tbody>
                      {learners?.map((learner: any, key) => {
                        const isLastRow = key === (learners?.length || 0) - 1;
                        const isSecondLastRow = key === (learners?.length || 0) - 2;
                        ...
                        <Table.Tr key={key}>
                          ...
                          <Table.Td>
                            <div className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${learner?.status === "L" || learner?.status === "G"
                              ...
                              {learner?.status === "Open" ? "Open" : ""}
                              {learner?.status === "In Progress" ? "In Progress" : ""}
                              {learner?.status === "Resolved" ? "Resolved" : ""}
                            </div>
                          </Table.Td>
              ...
              <div className="bg-white shadow-md rounded-lg  m-4">
                <div className="p-6">
                  ...
                  <div className="tab-content">
                    ...
                    {activeTab === "basicInfo" && (
                      <div className="tab-pane active bg-gray-50 p-4 rounded-lg shadow-inner">
                        {/* Top Bar */}
                        ...
                        <div className="border-b pb-3 mb-4">
                          ...
                          <div className="flex items-center justify-between mb-2">
                            <h1 className="text-lg font-semibold text-gray-800">
                              {learner.subject || "Untitled Complaint"}
                            </h1>

                            <span
                              className={`px-3 py-1 text-xs rounded-full ${learner.status === "Resolved"
                        ...
                        <div className="flex flex-col lg:flex-row gap-4">
                          {/* Left Side: Conversation and Reply */}
                          <div className="flex-1 bg-white rounded-lg border border-gray-200 shadow-sm">
                            ...
                            <div className="p-4 space-y-4 max-h-[450px] overflow-y-auto">
                              {learner.replies && learner.replies.length > 0 ? (
                                learner.replies.map((reply: any, index: number) => (
                                  <div
                                    key={index}
                                    className={`flex ${reply.sender === "Staff" ? "justify-end" : "justify-start"
                                      }`}
                                  >
                                    <div
                                      className={`max-w-md p-3 rounded-lg text-sm ${reply.sender === "Staff"
                            ...
                            <div className="border-t border-gray-200 p-4 bg-gray-50 rounded-b-lg">
                              ...
                              >
                                <textarea
                                  ...
                                  className="w-full border border-gray-300 rounded-lg p-2 text-sm focus:outline-none focus:ring focus:ring-indigo-200"
                                  rows={3}
                                ></textarea>
                          ...
                          {/* Right Side: Complaint Details Panel */}
