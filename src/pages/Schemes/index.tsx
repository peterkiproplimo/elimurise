...
function SchemeOfWorkSelectionBuilder() {
  ...
  const handleSave = (index) => {
    ...
    // Optionally, you can add API calls here to persist changes
  };
  ...
  const generateLessonsWithGrok = async () => {
    ...
    try {
      ...
      setSuccess(true);
      setMessage("Lessons generated successfully!");
      notify.current?.showToast();
    } catch (error) {
      setSuccess(false);
              ...
              <table className="w-full border-collapse border border-gray-300">
                ...
                <tbody>
                  {(() => {
                    ...
                    generatedLessons.forEach((row, idx) => {
                      const week = row.week;
                      if (weekRowSpanMap[week]) {
                        weekRowSpanMap[week].count += 1;
                        weekRowSpanMap[week].indexes.push(idx);
                      } else {
                        weekRowSpanMap[week] = { count: 1, indexes: [idx] };
                      }
                    });

                    return generatedLessons.map((row, index) => {
                      const weekData = weekRowSpanMap[row.week];
                      const isFirstWeekOccurrence =
                        weekData.indexes[0] === index;
                      ...
                      return (
                        ...
                        >
                          {isFirstWeekOccurrence && (
                            <td
                              className="border border-gray-300 p-2 text-t align-top"
                              rowSpan={weekData.count}
                            >
                              {row.week}
                            </td>
                          )}
                          ...
                          <td className="border border-gray-300 p-2">
                            {editingRowIndex === index ? (
                              <FormTextarea
                                value={row.keyInquiryQuestions.replace(
                                  /<\/?[^>]+(>|$)/g,
                                  ""
                                )}
                                onChange={(e) =>
                          ...
                          <td className="border border-gray-300 p-2">
                            {editingRowIndex === index ? (
                              <FormTextarea
                                value={row.learningExperiences.replace(
                                  /<\/?[^>]+(>|$)/g,
                                  ""
                                )}
                                onChange={(e) =>
                          ...
                          <td className="border border-gray-300 p-2">
                            {editingRowIndex === index ? (
                              <FormTextarea
                                value={row.learningResources.replace(
                                  /<\/?[^>]+(>|$)/g,
                                  ""
                                )}
                                onChange={(e) =>
